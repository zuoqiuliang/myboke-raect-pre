...
function PersonalProfile({ getFileList }: { getFileList: () => any[] }) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		e.preventDefault();
		try {
			// 调用 getUserInfoSidebar 组件的 getFileList 方法获取文件列表
	...
	return (
		<div className={personalProfileStyle.personal_profile_container}>
			...
			<form onSubmit={handleSubmit} className={personalProfileStyle.form}>
				...
				<div className={personalProfileStyle.form_item}>
					<label className={personalProfileStyle.form_label}>职业方向</label>
					<input
						type="text"
